...
class LammpsData:
    ...
    def __init__(self, name, symbol_indicies, masses, atoms, lammps_box, potentials=None):
        self.name = name
    ...
    @classmethod
    def from_structure(cls, structure, include_charge=False, potentials=None):
        ...
        return cls(name, symbol_indicies, masses, atoms, lammps_box, potentials)
    ...
    def from_file(cls, filename, atom_style=None):
        ...
        # Assume always in full format (change latter)
        atoms = []
        ...
        for s1, s2, *parameters in sections['PairIJ Coeffs']['data']:
            pair_potentials[(index_symbols[s1], index_symbols[s1])] = ' '.join(list(map(str, parameters)))

        potentials = LammpsPotentials(pair_potentials, symbol_indicies)

        return cls(description, symbol_indicies, masses, atoms, lammps_box, potentials)
    ...
    def __str__(self):
        return '\n'.join([
            ...
            '{}\n'.format(str(self.potentials)) if self.potentials else '',
        ])
